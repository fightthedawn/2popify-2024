...
def process_audio_file(file_path, ort_session, temp_folder, detection_threshold=0.5):
    try:
        ...
        if has_2pop:
            ...
            start_of_music = find_music_onset_from_audio_segment(audio_trimmed)
            ...
            start_of_music = find_music_onset_from_audio_segment(audio)
...
def find_music_onset_from_audio_segment(audio_segment):
    samples = np.array(audio_segment.get_array_of_samples())
    if audio_segment.channels == 2:
        samples = samples.reshape((-1, 2)).mean(axis=1)
    sr = audio_segment.frame_rate
    onset_frames = librosa.onset.onset_detect(y=samples, sr=sr, units='time', pre_max=1, post_max=1, pre_avg=1, post_avg=1, delta=0.01, wait=1)
    return int(onset_frames[0] * 1000) if onset_frames.size > 0 else 0
