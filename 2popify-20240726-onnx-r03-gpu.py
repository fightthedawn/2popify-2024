...
def detect_2_pop_with_model(audio_path, ort_session, detection_threshold=0.95):
    audio = AudioSegment.from_file(audio_path)[:1500]  # Get first 1.5 seconds
    samples = np.array(audio.get_array_of_samples())
    if audio.channels == 2:
        samples = samples.reshape((-1, 2)).mean(axis=1)
    samples = librosa.resample(samples, audio.frame_rate, 16000)
    samples = np.pad(samples, (0, max(0, 24000 - len(samples))))[:24000]
    
    inputs = {ort_session.get_inputs()[0].name: np.array([samples], dtype=np.float32)}
