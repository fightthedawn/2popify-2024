...
def process_audio_file(file_path, ort_session, temp_folder, detection_threshold=0.5):
    try:
        ...
        if has_2pop:
            ...
            actual_start_position = max(start_of_music - 500, 0)  # Ensure we have at least 500ms before music start
            ...
            print(f"Final trim point: {1500 + actual_start_position} ms, Silence added: {silence_duration} ms")
            ...
            print(f"Silence duration: {silence_duration} ms, Trim point: {max(0, start_of_music - 500)} ms")
