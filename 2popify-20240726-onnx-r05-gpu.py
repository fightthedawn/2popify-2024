...
def normalize_single_file(file_path, target_peak=-0.5):
...
def adjust_loudness(file_path, lowest_lufs):
    data, rate = sf.read(file_path)
    loudness = pyln.Meter(rate).integrated_loudness(data)
...
def normalize_audio(folder_path, target_peak=-0.5):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
        futures = [executor.submit(normalize_single_file, os.path.join(folder_path, f), target_peak) for f in audio_files]
        results = [future.result() for future in concurrent.futures.as_completed(futures)]
    
    lowest_lufs = min(result[0] for result in results)
    
    with concurrent.futures.ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
        futures = [executor.submit(adjust_loudness, result[1], lowest_lufs) for result in results]
        list(concurrent.futures.as_completed(futures))  # Wait for all tasks to complete
...
if __name__ == "__main__":
    ...
    # Profile the process_folder function
    pr = cProfile.Profile()
    pr.enable()
